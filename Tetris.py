...
currentDirectory = os.path.realpath(__file__)[:-10]
...
showFPS = False #shows frames per second if True

graying = False #turns immobile cells to gray if True

seed = None #determines randomness of bags. If None, seed is current time
random.seed(seed)

ghost = True #shows ghost piece if True

directionChangeCancel = False #Cancels das delay when switching directions if True

sdf = float("+inf") #soft drop factor
arr = 2 #automatic repeat rate
